...
export default function PromptEditor({ prompt, onSave, onCancel }: PromptEditorProps) {
  const [form, setForm] = useState<FormState>({
    ...
    outputType: prompt?.outputType || 'log',
    outputConfig: prompt?.outputConfig || {},
  ...
  const handleOutputConfigChange = (field: string, value: unknown) => {
    setForm((prev) => ({
      ...prev,
  ...
  const outputConfig = form.outputConfig as Record<string, string>;
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(val) => handleChange('outputType', val)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
